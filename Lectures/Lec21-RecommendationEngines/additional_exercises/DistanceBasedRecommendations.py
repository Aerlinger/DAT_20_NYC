...
def jaccard_similarity(movies_for_user_A, movies_for_user_B):
  """
          Returns a similarity for two arrays of movies for user A and user B
  """
  set_of_similar_movies = set(movies_for_user_A).intersection(set(movies_for_user_B))
  union_of_movies_for_both_users = set(movies_for_user_A).union(set(movies_for_user_B))

  return float(len(set_of_similar_movies))/float(len(union_of_movies_for_both_users))
